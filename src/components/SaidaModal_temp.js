            ...
            try {
                ...
                try {
                    const [tipoResponse, companyResponse, accountResponse] = await Promise.all([
                        fetch(`${API_BASE_URL}/tipo_SaÌda?projectId=${projectId}`, { headers: { 'Authorization': `Bearer ${token}` } }),
                    ...
                    if (tipoResponse.ok) {
                        ...
                        tipoSaÌdas = Array.isArray(json) ? json : [];
                    }
                ...
                modal.innerHTML = `
                    <div class="account-modal-body" style="padding: 1rem; overflow-y: hidden; max-height: 95vh;">
                        <h3 style="margin: 0 0 1rem 0; color: var(--color-primary); font-size: 1.1rem;">${isEdit ? 'Editar SaÌda' : 'Nova SaÌda'}</h3>
                        ...
                        <div class="form-grid" style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 0.75rem;">
                            ...
                            <div class="form-group" style="grid-column: span 3; display: flex; flex-direction: column;">
                                <label>Tipo de SaÌda <span class="required">*</span></label>
                                <div id="tree-selector-container" style="flex: 1; height: 200px;"></div>
                                <input type="hidden" id="saida-tipo-SaÌda-id" value="${saida?.tipo_SaÌda_id || ''}" />
                            </div>
                    ...
                    <div class="account-modal-footer" style="padding: 1rem; position: relative; z-index: 100;">
                        ...
                        <button class="btn-primary" id="modal-save" type="button">
                            ${isEdit ? 'Salvar Altera√ß√µes' : 'Criar SaÌda'}
                        </button>
                    </div>
                `;
                ...
                const tipoSaÌdaIdInput = modal.querySelector('#saida-tipo-SaÌda-id');
                ...
                const validate = () => {
                    ...
                    if (!tipoSaÌdaIdInput.value) { treeContainer.style.border = '1px solid #EF4444'; isValid = false; } else treeContainer.style.border = '1px solid var(--color-border-light)';
                ...
                const initialTipoId = parseInt(tipoSaÌdaIdInput.value);
                // Safe check ensures tipoSaÌdas is array
                TreeSelector.render(treeContainer, tipoSaÌdas, initialTipoId, (selectedId) => {
                    if (initialTipoId !== selectedId) {
                        hasChanges = true;
                    }
                    tipoSaÌdaIdInput.value = selectedId;
                    validate();
                });
                ...
                saveBtn.addEventListener('click', async (e) => {
                    ...
                    try {
                        ...
                        const data = {
                            ...
                            tipoSaÌdaId: parseInt(tipoSaÌdaIdInput.value),
